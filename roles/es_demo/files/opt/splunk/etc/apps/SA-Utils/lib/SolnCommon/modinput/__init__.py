...
class ModularInput(object):
    ...
    def get_checkpoint_data(self, filename, checkpoint_dir=None):
        """
        ...
        try:
            with open(os.path.join(checkpoint_dir, filename), 'r') as fp:
                data = json.load(fp)
        except IOError:
            logger.exception('msg="IOError exception when reading checkpoint data" checkpoint_dir="{}" filename="{}"'.format(checkpoint_dir, filename))
        except ValueError:
            logger.exception('msg="ValueError when reading checkpoint data (perhaps invalid JSON)" checkpoint_dir="{}" filename="{}"'.format(checkpoint_dir, filename))
        except Exception:
    ...
    def get_checkpoint_update_times(self, stanza_names, modinput_name=None):
        ...
        for stanza_name in stanza_names:
            ...
            if os.path.isfile(path):
                try:
                    output.append((stanza_name, path, int(os.stat(path).st_mtime)))
                except IOError:
                    output.append((stanza_name, path, None))
            else:
                output.append((stanza_name, None, None))
        return output
